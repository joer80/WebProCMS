...
new #[Layout('layouts.public', ['description' => 'Find a GetRows office near you. We have locations across the South and Midwest including Dallas, Houston, Little Rock, Fayetteville, and Oklahoma City.'])] #[Title('Locations — GetRows')] class extends Component {
    ...
    // ROW:php:start:locations-grid:Emx5Ke
    ...
    // ROW:php:end:locations-grid:Emx5Ke
}; ?>
<div>{{-- ROW:start:locations-grid:Emx5Ke --}}
<x-dl.section slug="locations-grid:Emx5Ke"
    default-section-classes="py-section px-6 bg-white dark:bg-zinc-900"
    default-container-classes="max-w-6xl mx-auto">
    <x-dl.heading slug="locations-grid:Emx5Ke" prefix="headline" default="Our Locations"
        default-tag="h2"
        default-classes="font-heading text-4xl font-bold text-zinc-900 dark:text-white mb-4" />
    <x-dl.subheadline slug="locations-grid:Emx5Ke" prefix="subheadline" default="Find a location near you."
        default-classes="text-zinc-500 dark:text-zinc-400 leading-normal mb-10" />

    <x-dl.wrapper slug="locations-grid:Emx5Ke" prefix="filter_wrapper" default-classes="flex flex-wrap items-center gap-2 mb-8">
        <x-dl.wrapper slug="locations-grid:Emx5Ke" prefix="filter_button" tag="button"
    ...
    @if ($this->locationsFiltered->isNotEmpty())
        <x-dl.wrapper slug="locations-grid:Emx5Ke" prefix="locations_grid" default-classes="grid sm:grid-cols-3 gap-6"
            ...
            @foreach ($this->locationsFiltered as $location)
                <x-dl.card slug="locations-grid:Emx5Ke" prefix="location_card"
                    ...
                    @if ($location->photoUrl())
                        <x-dl.wrapper slug="locations-grid:Emx5Ke" prefix="location_image" tag="img"
                            src="{{ $location->photoUrl() }}"
                            alt="{{ $location->name }}"
                            default-classes="w-full h-48 object-cover" />
                    @endif
                    <x-dl.group slug="locations-grid:Emx5Ke" prefix="card_content" default-classes="p-5">
                        <x-dl.wrapper slug="locations-grid:Emx5Ke" prefix="location_name" tag="h3"
                            default-classes="font-semibold text-base text-zinc-900 dark:text-white mb-3">
                            {{ $location->name }}
                        </x-dl.wrapper>
                        <x-dl.wrapper slug="locations-grid:Emx5Ke" prefix="location_details"
                            default-classes="space-y-1.5 text-sm text-zinc-500 dark:text-zinc-400 mb-5">
                            <p>{{ $location->address }}</p>
                            <p>{{ $location->city }}, {{ $location->state }} {{ $location->zip }}</p>
                            <p>{{ $location->phone }}</p>
                        </x-dl.wrapper>
                        <x-dl.wrapper slug="locations-grid:Emx5Ke" prefix="directions_link" tag="a"
