...
new #[Layout('layouts.public', ['description' => 'Browse our upcoming and past events.'])] #[Title('Events')] class extends Component {
    public string $pageName = 'Events';

    // ROW:php:start:events-list:placeholder
    ...
    {
        $this->eventViewMode = in_array($mode, ['list', 'month', 'day']) ? $mode : 'list';
        $this->resetPage();
    }

    public function setEventTab(string $tab): void
    {
        $this->eventTab = in_array($tab, ['upcoming', 'past']) ? $tab : 'upcoming';
        $this->resetPage();
    }
    ...
    public function getEventsDayProperty(): \Illuminate\Database\Eloquent\Collection
    {
        ...
        return \App\Models\Event::query()
            ->published()
            ->parent()
            ->whereDate('start_date', $this->eventSelectedDay)
            ->orderBy('start_date')
            ->get();
    }
    // ROW:php:end:events-list:placeholder
}; ?>
<div>{{-- ROW:start:page-title-banner:events-banner --}}
<x-dl.section slug="page-title-banner:events-banner"
    default-section-classes="relative py-section-banner px-6 bg-zinc-800 bg-cover bg-center"
    default-container-classes="max-w-6xl mx-auto">
    <x-dl.wrapper slug="page-title-banner:events-banner" prefix="overlay" tag="div"
        default-toggle="1"
        default-classes="absolute inset-0 bg-black/50" />
    <x-dl.heading slug="page-title-banner:events-banner" prefix="headline" default="{{ $pageName ?? 'Page Title' }}"
        default-tag="h1"
        default-classes="relative z-10 font-heading text-4xl sm:text-5xl font-bold text-white" />
</x-dl.section>

{{-- ROW:end:page-title-banner:events-banner --}}

{{-- ROW:start:events-list:placeholder --}}
<x-dl.section slug="events-list:placeholder"
    default-section-classes="py-section px-6 bg-white dark:bg-zinc-900"
    default-container-classes="max-w-6xl mx-auto">
    <x-dl.heading slug="events-list:placeholder" prefix="headline" default="Events"
        default-tag="h1"
        default-classes="text-4xl font-semibold leading-tight mb-4" />
    <x-dl.subheadline slug="events-list:placeholder" prefix="subheadline" default="Browse our upcoming and past events."
        default-classes="text-zinc-500 dark:text-zinc-400 leading-normal mb-10" />

    {{-- Controls row: view switcher (left) + search/tabs (right, list-only) --}}
    <x-dl.wrapper slug="events-list:placeholder" prefix="controls_row"
        default-classes="grid grid-cols-1 md:grid-cols-2 gap-4 items-center mb-8">
        <x-dl.wrapper slug="events-list:placeholder" prefix="view_switcher"
            default-classes="flex items-center gap-2">
            <x-dl.wrapper slug="events-list:placeholder" prefix="view_list_btn" tag="button"
            ...
            <x-dl.wrapper slug="events-list:placeholder" prefix="view_month_btn" tag="button"
            ...
            <x-dl.wrapper slug="events-list:placeholder" prefix="view_day_btn" tag="button"
        ...
        @if ($eventViewMode === 'list')
            <x-dl.wrapper slug="events-list:placeholder" prefix="list_filters"
                default-classes="flex flex-col sm:flex-row sm:items-center sm:justify-end gap-3">
                <x-dl.group slug="events-list:placeholder" prefix="list_search_wrapper"
                    ...
                    <x-dl.wrapper slug="events-list:placeholder" prefix="list_search_input" tag="input"
                ...
                <x-dl.group slug="events-list:placeholder" prefix="list_tab_buttons"
                    default-classes="flex items-center gap-2 shrink-0">
                    <x-dl.wrapper slug="events-list:placeholder" prefix="list_tab_upcoming" tag="button"
                    ...
                    <x-dl.wrapper slug="events-list:placeholder" prefix="list_tab_past" tag="button"
    ...
    @if ($eventViewMode === 'list')
        @if ($this->eventsList->isEmpty())
            <x-dl.wrapper slug="events-list:placeholder" prefix="list_empty" tag="p"
                default-classes="text-zinc-500 dark:text-zinc-400 text-sm">
                @if ($eventSearch)
                    No events found for "{{ $eventSearch }}".
                @else
                    No {{ $eventTab }} events found.
                @endif
            </x-dl.wrapper>
        @else
            <x-dl.wrapper slug="events-list:placeholder" prefix="list_items"
                default-classes="space-y-4">
                @foreach ($this->eventsList as $event)
                    <x-dl.card slug="events-list:placeholder" prefix="list_item" tag="a"
                        ...
                        <x-dl.group slug="events-list:placeholder" prefix="list_date_badge"
                            ...
                            <x-dl.wrapper slug="events-list:placeholder" prefix="list_date_month" tag="span"
                                default-classes="text-xs font-semibold uppercase leading-none">
                                {{ $event->start_date->format('M') }}
                            </x-dl.wrapper>
                            <x-dl.wrapper slug="events-list:placeholder" prefix="list_date_day" tag="span"
                                default-classes="text-2xl font-bold leading-none">
                                {{ $event->start_date->format('j') }}
                            </x-dl.wrapper>
                        </x-dl.group>
                        <x-dl.group slug="events-list:placeholder" prefix="list_item_content"
                            default-classes="flex-1 min-w-0">
                            <x-dl.wrapper slug="events-list:placeholder" prefix="list_item_title" tag="h3"
                            ...
                            <x-dl.wrapper slug="events-list:placeholder" prefix="list_item_meta" tag="p"
                            ...
                            @if ($event->excerpt)
                                <x-dl.wrapper slug="events-list:placeholder" prefix="list_item_excerpt" tag="p"
                                    default-classes="text-sm text-zinc-500 dark:text-zinc-400 mt-1 line-clamp-1">
                                    {{ $event->excerpt }}
                                </x-dl.wrapper>
                            @endif
                        </x-dl.group>
                        <x-dl.wrapper slug="events-list:placeholder" prefix="list_item_arrow" tag="span"
            ...
            <x-dl.wrapper slug="events-list:placeholder" prefix="list_pagination"
    ...
    @if ($eventViewMode === 'month')
        @php
            ...
            $eventsJson = collect($this->eventsMonth)->map(fn ($e) => [
                'title' => $e->title,
                'slug' => $e->slug,
        ...
        <x-dl.wrapper slug="events-list:placeholder" prefix="month_header"
            default-classes="flex items-center justify-between mb-4">
            <x-dl.wrapper slug="events-list:placeholder" prefix="month_title" tag="h2"
                default-classes="text-lg font-semibold text-zinc-900 dark:text-zinc-100">
                {{ $monthCarbon->format('F Y') }}
            </x-dl.wrapper>
            <x-dl.group slug="events-list:placeholder" prefix="month_nav_buttons"
                default-classes="flex items-center gap-2">
                <x-dl.wrapper slug="events-list:placeholder" prefix="month_prev_btn" tag="button"
                ...
                <x-dl.wrapper slug="events-list:placeholder" prefix="month_next_btn" tag="button"
        ...
        <x-dl.wrapper slug="events-list:placeholder" prefix="month_dow_header"
            default-classes="grid grid-cols-7 mb-1">
            @foreach (['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'] as $dayName)
                <div class="text-xs font-medium text-zinc-500 dark:text-zinc-400 text-center py-2">
                    {{ $dayName }}
                </div>
            @endforeach
        </x-dl.wrapper>
        ...
        {{-- Calendar grid --}}
        <x-dl.wrapper slug="events-list:placeholder" prefix="month_grid"
            ...
            @foreach ($cells as $cell)
                ...
                @endphp
                <div class="min-h-24 p-1 border-r border-b border-zinc-200 dark:border-zinc-700 {{ $isCurrentMonth ? 'bg-white dark:bg-zinc-900' : 'bg-zinc-50 dark:bg-zinc-800/30' }}">
                    <span class="inline-flex items-center justify-center size-6 text-xs font-medium mb-1 {{ $isToday ? 'rounded-full bg-primary text-white' : ($isCurrentMonth ? 'text-zinc-900 dark:text-zinc-100' : 'text-zinc-400 dark:text-zinc-600') }}">
                        {{ $cell->day }}
                    </span>
                    @foreach ($dayEvents as $ev)
                        <a href="{{ route('events.show', $ev['slug']) }}"
                            class="block truncate text-xs px-1 py-0.5 rounded bg-primary/10 text-primary hover:bg-primary/20 transition-colors mb-0.5">
                            {{ $ev['title'] }}
                        </a>
                    @endforeach
                </div>
    ...
    @if ($eventViewMode === 'day')
        <x-dl.wrapper slug="events-list:placeholder" prefix="day_picker_wrapper"
            default-classes="mb-6">
            <x-dl.wrapper slug="events-list:placeholder" prefix="day_picker_label" tag="label"
                default-classes="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">
                Select a date
            </x-dl.wrapper>
            <x-dl.wrapper slug="events-list:placeholder" prefix="day_picker_input" tag="input"
        ...
        @if ($eventSelectedDay)
            @if ($this->eventsDay->isEmpty())
                <x-dl.wrapper slug="events-list:placeholder" prefix="day_empty" tag="p"
                    default-classes="text-zinc-500 dark:text-zinc-400 text-sm">
                    No events on {{ \Carbon\Carbon::parse($eventSelectedDay)->format('l, F j, Y') }}.
                </x-dl.wrapper>
            @else
                <x-dl.wrapper slug="events-list:placeholder" prefix="day_items"
                    default-classes="space-y-3">
                    @foreach ($this->eventsDay as $event)
                        <x-dl.card slug="events-list:placeholder" prefix="day_item" tag="a"
                            ...
                            <x-dl.wrapper slug="events-list:placeholder" prefix="day_item_time" tag="span"
                                default-classes="text-sm font-medium text-primary w-20 shrink-0">
                                @if ($event->is_all_day)
                                    All Day
                                @else
                                    {{ $event->start_date->format('g:i A') }}
                            ...
                            <x-dl.wrapper slug="events-list:placeholder" prefix="day_item_title" tag="span"
                            ...
                            @if ($event->venue_name)
                                <x-dl.wrapper slug="events-list:placeholder" prefix="day_item_venue" tag="span"
            ...
            @endif
        @else
            <x-dl.wrapper slug="events-list:placeholder" prefix="day_prompt" tag="p"
                default-classes="text-zinc-500 dark:text-zinc-400 text-sm">
...
{{-- ROW:end:events-list:placeholder --}}
</div>
