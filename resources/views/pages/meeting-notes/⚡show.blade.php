...
new #[Layout('layouts.public')] class extends Component {
    // ROW:php:start:content-show:lahYBK
    ...
    // ROW:php:end:content-show:lahYBK
}; ?>
<div>
{{-- ROW:start:content-show:lahYBK --}}
<x-dl.section slug="content-show:lahYBK"
    default-section-classes="py-section px-6 bg-white dark:bg-zinc-900"
    default-container-classes="max-w-3xl mx-auto">

    <x-dl.wrapper slug="content-show:lahYBK" prefix="breadcrumb" tag="nav"
    ...
    <x-dl.wrapper slug="content-show:lahYBK" prefix="item_title" tag="h1"
        default-classes="font-heading text-4xl font-bold text-zinc-900 dark:text-white mb-2">
        {{ $item->displayTitle() }}
    </x-dl.wrapper>

    @if ($item->published_at)
        <x-dl.wrapper slug="content-show:lahYBK" prefix="item_date" tag="p"
            default-classes="text-sm text-zinc-500 dark:text-zinc-400 mb-8">
            {{ $item->published_at->format('F j, Y') }}
        </x-dl.wrapper>
    @endif

    <x-dl.wrapper slug="content-show:lahYBK" prefix="back_link" tag="div"
