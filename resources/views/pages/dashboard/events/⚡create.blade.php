...
new #[Layout('layouts.app')] #[Title('New Event')] class extends Component {
    use WithFileUploads;
    ...
    {
        $this->eventTimezone = \App\Models\Setting::get('site.timezone', date_default_timezone_get());
    }
    ...
    private function performSave(): Event
    {
        $this->validate();
    ...
    {
        $event = $this->performSave();
        $this->redirect(route('dashboard.events.edit', $event), navigate: true);
    }
    ...
    {
        $event = $this->performSave();
        $this->redirect(route('events.show', $event->slug));
    }
...
<div>
    <flux:main>
        <div class="mb-8 flex items-center gap-4">
            ...
            <flux:heading size="xl">New Event</flux:heading>
        </div>
                                ...
                                <div x-data="{ ogOpen: false }" class="pt-4 border-t border-zinc-200 dark:border-zinc-700">
                                    ...
                                    <div x-show="ogOpen" x-transition class="mt-4 space-y-4">
                                        <flux:field>
                                            ...
                                            <flux:input wire:model="ogTitle" type="text" placeholder="Defaults to meta title or event title…" />
