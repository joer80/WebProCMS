...
new #[Layout('layouts.app')] #[Title('New Item')] class extends Component {
    use WithFileUploads;
    ...
    /** @var array<string, mixed> Temporary upload for gallery add */
    ...
    private function performSave(): ContentItem
    {
        $rules = [
    ...
    public function save(bool $exit = false): void
    {
        $item = $this->performSave();

        if ($exit) {
            $this->redirect(route('dashboard.content.index', $this->typeSlug), navigate: true);
        } else {
            $this->redirect(route('dashboard.content.edit', [$this->typeSlug, $item->id]), navigate: true);
        }
    }
...
<div>
    <flux:main>
        <div class="mb-8 flex items-center gap-4">
            ...
            <flux:heading size="xl">New {{ $typeDef->singular }}</flux:heading>
        </div>
