@blaze
@props(['slug', 'prefix' => 'nav', 'defaultMenu' => 'main-navigation', 'defaultAccordion' => '', 'defaultClasses' => '', 'defaultItemClasses' => '', 'defaultActiveItemClasses' => '', 'defaultDropdownClasses' => '', 'defaultDropdownItemClasses' => '', 'defaultAccordionItemClasses' => ''])
@php
$toggle = content($slug, "toggle_{$prefix}", '1');
$menuSlug = content($slug, "{$prefix}_menu", $defaultMenu);
$accordion = (bool) content($slug, "{$prefix}_accordion", $defaultAccordion);
...
$accordionItemClasses = content($slug, "{$prefix}_accordion_item_classes", $defaultAccordionItemClasses ?: ('pl-4 ' . $itemClasses));
...
<nav {{ $attributes }} class="{{ $navClasses }}"{!! $extraAttrsStr !!}>
    @foreach($navItems as $item)
        @if(($item['type'] ?? null) === 'mega')
            ...
            @if($accordion)
                {{-- Accordion: mega menu renders as collapsible sections --}}
                <div x-data="{ open: false }">
                    <button type="button" @click="open = !open"
                            class="{{ $itemClasses }} flex w-full cursor-pointer items-center justify-between gap-1.5">
                        <span>{{ $item['label'] }}</span>
                        <svg xmlns="http://www.w3.org/2000/svg" :class="open ? 'rotate-180' : ''" class="size-3 shrink-0 transition-transform" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M5.22 8.22a.75.75 0 0 1 1.06 0L10 11.94l3.72-3.72a.75.75 0 1 1 1.06 1.06l-4.25 4.25a.75.75 0 0 1-1.06 0L5.22 9.28a.75.75 0 0 1 0-1.06Z" clip-rule="evenodd" /></svg>
                    </button>
                    <div x-show="open"
                        ...
                        @foreach($megaColumns as $column)
                            @if(!empty($column['heading']))
                                <p class="mt-3 mb-1 text-xs font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-500">{{ $column['heading'] }}</p>
                            @endif
                            @foreach(($column['links'] ?? []) as $link)
                                <a href="{{ $link['url'] ?? '#' }}"
                                   @if(!empty($link['new_tab'])) target="_blank" rel="noopener noreferrer" @endif
                                   class="{{ $accordionItemClasses }}">{{ $link['title'] }}</a>
                            @endforeach
                        @endforeach
                    </div>
                </div>
                ...
                <div x-data="{ open: false }">
                    ...
                    <div x-show="open"
                        ...
                        @foreach($item['_children'] as $child)
                            <a href="{{ $child['url'] }}" class="{{ $accordionItemClasses }}">{{ $child['label'] }}</a>
                        @endforeach
                        @if(!empty($item['show_all']) && !empty($item['_see_all_url']))
                            <a href="{{ $item['_see_all_url'] }}" class="{{ $accordionItemClasses }} font-medium">
        ...
        @elseif(!empty($item['_sub_children']))
            @if($accordion)
                {{-- Accordion: manually nested sub-items --}}
                <div x-data="{ open: false }">
                    <button type="button" @click="open = !open"
                            class="{{ $itemClasses }} flex w-full cursor-pointer items-center justify-between gap-1.5">
                        <span>{{ $item['label'] }}</span>
                    ...
                    <div x-show="open"
                        ...
                        @foreach($item['_sub_children'] as $child)
                            ...
                            <a href="{{ $childHref }}"
                               ...
                               class="{{ $accordionItemClasses }}">{{ $child['label'] }}</a>
                        @endforeach
                    </div>
                </div>
            @else
                {{-- Dropdown: manually nested sub-items --}}
                <div class="relative inline-flex items-center" x-data="{ open: false }">
                    <button type="button" @click="open = !open" @click.outside="open = false"
                            class="{{ $itemClasses }} inline-flex cursor-pointer items-center gap-1.5">
                        {{ $item['label'] }}
