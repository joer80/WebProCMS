...
<section class="bg-primary py-16 px-6 text-center">
    <div class="max-w-3xl mx-auto">
        <h2 class="text-4xl font-bold text-white">{{ content('__SLUG__', 'headline', 'Ready to Get Started?', 'text', 'content') }}</h2>
        <p class="mt-4 text-lg text-white/80">{{ content('__SLUG__', 'subheadline', 'Join thousands of satisfied customers today.', 'text', 'content') }}</p>
        <div class="mt-8 flex flex-wrap items-center justify-center gap-4">
            ...
            >
                {{ content('__SLUG__', 'secondary_cta', 'Talk to Sales', 'text', 'call to action') }}
            </a>
            @endif
        </div>
    </div>
