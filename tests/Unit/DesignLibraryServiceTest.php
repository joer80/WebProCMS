...
it('strips @php block from parsed blade content', function (): void {
    $file = tempnam(sys_get_temp_dir(), 'dltest_').'.blade.php';
    file_put_contents($file, <<<'BLADE'
{{--
@name Hero With PHP
...
BLADE);
    ...
    expect($data)->not->toHaveKey('php_code');
...
BLADE;
    ...
    $data = $this->service->parseTemplateFile($file);
    $data['blade_code'] = "<section class=\"py-20\">\n    <h1>Hello World</h1>\n</section>";
...
BLADE);
    ...
    expect($keys[0])->toBe('section_classes')
        ...
        ->and($keys[2])->toBe('section_animation')
        ->and($keys[3])->toBe('section_animation_delay')
        ->and($keys[4])->toBe('section_bg_image')
        ->and($data['schema_fields'][4]['type'])->toBe('image')
        ->and($keys[5])->toBe('section_bg_position')
        ->and($data['schema_fields'][5]['type'])->toBe('bg_position')
        ->and($keys[6])->toBe('section_bg_size')
        ->and($data['schema_fields'][6]['type'])->toBe('bg_size')
        ->and($keys[7])->toBe('section_bg_repeat')
        ->and($data['schema_fields'][7]['type'])->toBe('bg_repeat')
        ->and($keys[8])->toBe('section_id')
        ->and($data['schema_fields'][8]['type'])->toBe('id')
        ->and($keys[9])->toBe('section_attrs')
        ->and($data['schema_fields'][9]['type'])->toBe('attrs')
        ->and($keys[10])->toBe('toggle_headline')
        ->and($data['schema_fields'][10]['type'])->toBe('toggle')
        ->and($keys[11])->toBe('headline_htag')
        ->and($data['schema_fields'][11]['default'])->toBe('h1')
        ->and($keys[12])->toBe('headline')
        ->and($data['schema_fields'][12]['default'])->toBe('Your Headline')
        ->and($keys[13])->toBe('headline_classes')
        ->and($data['schema_fields'][13]['type'])->toBe('classes')
        ->and($data['schema_fields'][13]['default'])->toBe('font-heading text-5xl font-bold text-zinc-900')
        ->and($keys[14])->toBe('headline_animation')
        ->and($data['schema_fields'][14]['type'])->toBe('animation')
        ->and($keys[15])->toBe('headline_animation_delay')
        ->and($data['schema_fields'][15]['type'])->toBe('animation_delay')
        ->and($keys[16])->toBe('headline_id')
        ->and($data['schema_fields'][16]['type'])->toBe('id');
