...
BLADE);
    ...
    expect($keys[0])->toBe('section_classes')
        ...
        ->and($keys[4])->toBe('section_id')
        ->and($data['schema_fields'][4]['type'])->toBe('id')
        ->and($keys[5])->toBe('section_attrs')
        ->and($data['schema_fields'][5]['type'])->toBe('attrs')
        ->and($keys[6])->toBe('toggle_headline')
        ->and($data['schema_fields'][6]['type'])->toBe('toggle')
        ->and($keys[7])->toBe('headline_htag')
        ->and($data['schema_fields'][7]['default'])->toBe('h1')
        ->and($keys[8])->toBe('headline')
        ->and($data['schema_fields'][8]['default'])->toBe('Your Headline')
        ->and($keys[9])->toBe('headline_classes')
        ->and($data['schema_fields'][9]['type'])->toBe('classes')
        ->and($data['schema_fields'][9]['default'])->toBe('font-heading text-5xl font-bold text-zinc-900')
        ->and($keys[10])->toBe('headline_animation')
        ->and($data['schema_fields'][10]['type'])->toBe('animation')
        ->and($keys[11])->toBe('headline_animation_delay')
        ->and($data['schema_fields'][11]['type'])->toBe('animation_delay')
        ->and($keys[12])->toBe('headline_id')
        ->and($data['schema_fields'][12]['type'])->toBe('id');
